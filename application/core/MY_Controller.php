...
class MY_Controller extends CI_Controller {
			...
			foreach ($all_action_elements as $this_element)
			{
				...
				if (!isset($mfunction['stop'])) {
			        ...
			        if (!$this_element['ajax'] || $this->url['subaction'] == 'ss'){
			            ...
			            $this_element_thisid_en = $getthisid["en"][$this_element_thisid];
			            }else{
			            $this_element_thisid_en = $this->url['id_plain'];
			            }
					...
					if ($output[$count_output]['isoutput']){
						...
						if (isset($output[$count_output]['isdiv'])){
				            if ($this_element['langname']) {
				            $output[$count_output]['div']['title'] = $this->lang->line('coreename_'.$this_element['langname']);;
				            } else {
	...
	function output_fullpage(){
		
		$app = $this->url['app'];
		$layout = $this->layout;
		$output = $this->data;
		
		$pagedata = array("title" => "", "css" => "", "js" => "", "js_bodyend" => "");
	...
	function output_content($layout, $output){
	//print_r($output);
	...
	foreach ($output as $this_output) {
		...
		$result['jsonload'] .= 'apps_action_ajax("'.$this_output['ajax']['app'].'","'.$this_output['ajax']['an'].'","'.$this_output['ajax']['subaction'].'","'.$this_output['ajax']['elementid'].'","'.$this_output['ajax']['id'].'");';
		}
	    ...
	    /////////////////////////////////////////////////////////
	    if (!$layout['content']) {
	        $sql3 = "SELECT * FROM core_layout_content WHERE core_layout_content_name  = 'full'";
	    }else{
	        $sql3 = "SELECT * FROM core_layout_content WHERE core_layout_content_name  = '".$layout['content']."'";
	    }
	...
	function output_page_divstyle($action_layout_name){
	
	    $sql_actlayout = "SELECT * FROM core_apps_action_layout WHERE core_apps_action_layout_name  = '$action_layout_name'";
	
	    $result = $this->db->query($sql_actlayout);
	...
	function output_page_format_tgrid($this_output,$gridnum,$this_divstyle){
	/*
	    $this_divstyle['style']
	    $this_divstyle['title']
	    $this_divstyle['drag']
	...
	*/
	    ...
	    $h_html .= '
			...
			if (isset($this_output['html'])) $h_html .= $this_output['html'];
	...
	function output_page_format_tgrid_tab($this_output,$build_tab,$gridnum){
	...
	function output_page_format_tgrid_tabwarpup($build_tab){
	...
	function output_page_format_simple($this_output,$gridnum,$this_divstyle){
