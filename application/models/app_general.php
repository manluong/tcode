...
class App_general extends CI_Model {
...
function core_app_getaccessgp($cardid){
    ...
    case "2":
        ...
        if ($result) {
        $thisresult["staffid"] = $result;
        $thisresult["en"]["staffid"] = f_thisid_encode($result);
        }
        break;
    case "3":
        //client
        ...
        $result = $db->fetchOne($sql, 2);
        if ($result) {
        $thisresult["clientid"] = $result;
        $thisresult["en"]["clientid"] = f_thisid_encode($result);
        }
        break;
    case "4":  break;
    case "5":
        ...
        $result = $db->fetchOne($sql, 2);
        if ($result) {
        $thisresult["vendorid"] = $result;
        $thisresult["en"]["vendorid"] = f_thisid_encode($result);
        }
        break;
    case "6":  break;
    case "7":  break;
    case "8":
        $sql = "SELECT spu_id FROM spu WHERE spu_cardid = ".$cardid." LIMIT 1";
        $result = $db->fetchOne($sql, 2);
        if ($result) {
        $thisresult["spuid"] = $result;
        $thisresult["en"]["spuid"] = f_thisid_encode($result);
        }
        break;
    }
...
function core_app_getthisid($thisidtype,$thisid,$card_accessgp){

    global $db,$thisid_en,$shortaved,$id;
    //this function will get an thisid, and return all other id in an array
    switch($thisidtype){

    case "client":
        $sql = "SELECT client_cardid FROM client WHERE client_id = ".$thisid[0]." LIMIT 1";
        $result = $db->fetchOne($sql, 2);
        if ($result) {
        $getthisid["cardid"] = $result;
        $getthisid["en"]["cardid"] = f_thisid_encode($result);
        }
        $getthisid["clientid"] = $thisid[0];
        $getthisid["en"]["clientid"] = $thisid_en;
    break;

    case "staff":
        $sql = "SELECT staff_cardid FROM staff WHERE staff_id = ".$thisid[0]." LIMIT 1";
        $result = $db->fetchOne($sql, 2);
        if ($result) {
        $getthisid["cardid"] = $result;
        $getthisid["en"]["cardid"] = f_thisid_encode($result);
        }
        $getthisid["staffid"] = $thisid[0];
        $getthisid["en"]["staffid"] = $thisid_en;
    break;

    case "vendor":
        $sql = "SELECT vendor_cardid FROM vendor WHERE vendor_id = ".$thisid[0]." LIMIT 1";
        $result = $db->fetchOne($sql, 2);
        if ($result) {
        $getthisid["cardid"] = $result;
        $getthisid["en"]["cardid"] = f_thisid_encode($result);
        }
        $getthisid["vendorid"] = $thisid[0];
        $getthisid["en"]["vendorid"] = $thisid_en;
    break;


    case "card":

        $getthisid = core_app_getaccessgp($thisid[0]);

        /*
        //if $app is card, go get the access_group to see if it's client/vendor/staff
        if (!$card_accessgp && $thisid[0]){
        $sql = "SELECT access_link_gpmaster FROM access_link WHERE access_link_cardid = ".$thisid[0]." LIMIT 1";
        $card_accessgp = $db->fetchOne($sql, 2);
        }
        ...
        case "2":  break;
        case "3":
            $sql = "SELECT client_id FROM client WHERE client_cardid = ".$thisid[0]." LIMIT 1";
            $result = $db->fetchOne($sql, 2);
            if ($result) {
            $getthisid["clientid"] = $result;
        ...
        */
        ...
        $getthisid["cardid"] = $thisid[0];
        $getthisid["en"]["cardid"] = $thisid_en;
    break;

    case "invoice":

        $sql = "SELECT a_invoice_clientid FROM a_invoice WHERE a_invoice_id = ".$thisid[0]." LIMIT 1";
        ...
        $getthisid["clientid"] = $clientid;
        $getthisid["en"]["clientid"] = f_thisid_encode($clientid);
    ...
    case "invoiceitem":
        ...
        if($shortaved == "a"){
            ...
            $sql = "SELECT a_invoice_item_invid FROM a_invoice_item WHERE a_invoice_item_id = ".$thisid[0]." LIMIT 1";
    ...
    case "spuid":
        ...
        if ($id["spuid"]){
            $getthisid["spuid"] = $id["spuid"];
            $getthisid["en"]["spuid"] = f_thisid_encode($id["spuid"]);

            $sql = "SELECT spu_cardid FROM spu WHERE spu_id = ".$getthisid["spuid"]." LIMIT 1";
            $result = $db->fetchOne($sql, 2);
            if ($result) {
            $getthisid["cardid"] = $result;
            $getthisid["en"]["cardid"] = f_thisid_encode($result);
